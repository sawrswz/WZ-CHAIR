...
static vec4_t asVec4V(float x, float y, float z, float w) { vec4_t out; out.x = x; out.y = y; out.z = z; out.w = w; return out; }
struct Matrix4x4 { union { vec4_t v[4]; float m[4][4]; struct { vec4_t right; vec4_t up; vec4_t forward; vec4_t trans; }; }; };
